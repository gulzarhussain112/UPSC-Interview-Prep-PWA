...
function trackEntry(statusMap, slot){
  for(const d of Object.keys(SCHEDULE).sort()){
    ...
    if(e && statusMap[`${e[1]}||${e[2]}`] !== 'done') return e;
  }
  return null;
}

function sessionId(date,slot){ return `${date}__slot${slot}`; }
...
async function sendForUser(userDoc){
  ...
  if(!u.enabled || !u.fcmToken || !u.timezone) return;
  ...
  for(const slotInfo of SLOTS){
    ...
    // Opening the PWA during the session marks it acknowledged and stops reminders.
    if(session.acknowledged) continue;

    const elapsed=now.minutes-slotInfo.start;
    let stage=null;
    if(elapsed>=0 && elapsed<slotInfo.end-slotInfo.start){
      if(elapsed<30) stage=0;
      else if(elapsed<60) stage=30;
      else stage=60;
    ...
    if(session.lastReminderStage===stage) continue;
    ...
    try{
      ...
      console.log('Sent',msg.title,'to',userDoc.id,id);
    }catch(err){
      console.error('FCM error for',userDoc.id,err.code||err.message);
      if(['messaging/registration-token-not-registered','messaging/invalid-registration-token'].includes(err.code)){
        await userDoc.ref.set({enabled:false},{merge:true});
      }
    }
  }
}
...
(async()=>{
  ...
  console.log(`Checking ${users.size} users at ${new Date().toISOString()}`);
  for(const doc of users.docs) await sendForUser(doc);
})();
